...
<!--start of header name-->
 <h1 class="h3 mb-4 text-gray-800">BASIC DETAILS</h1>
         <?php 
...
if(isset($_REQUEST['save']))
{
  ...
  //end of of varibale and value setting
	...
	$query = "INSERT INTO `basic_details`(`user_code`,`name`,`address`,`email`,`mobile`,`profile`,`objective`,`declaration`)VALUES('$user_code','$name','$address','$email','$mobile','$profile','$objective','$declaration')";  
...
if(isset($_REQUEST['edit'])){
  ...
  //end of of varibale and value setting
  //start of typing  query
	$query = "UPDATE `basic_details` SET `user_code`='$user_code',`name`='$name',`address`='$address',`email`='$email',`mobile`='$mobile',`profile`='$profile',`objective`='$objective',`declaration`='$declaration' WHERE id='$id'";   
  // end of typing  query
	$message=iud_data($query);
  if($message=="sucess"){
		echo"<script>alert('data saved successfully ')</script>";
...
if($action == 'delete'){
	$id   = $_REQUEST['id'];
	//start of typing  query
	$query = "delete from basic_details where id='$id'";  
  //end of typing  query
	$message=iud_data($query);
  if($message=="sucess"){
...
<div class="container">
  ...
  <?php if($action == 'edit' or $action == 'add' ){ 
          $id = $_REQUEST['id'];
        //start of typing  query
		  $query ="select * from basic_details where id='$id'";
      //end of typing  query
  ...
  <td>
   <input type="text" name="user_code" id="user_code" class="form-control"value="<?php if($action=="edit"){ echo $data['user_code'];}else {echo "";} ?>" placeholder="user_code" required="required" autofocus="autofocus">
  </td>
  </tr>
  <tr>
  <td align="right"><b>name</b></td>
  <td>
  <input type="text" name="name" class="form-control" value="<?php if($action=="edit"){echo $data['name'];}else{echo "";} ?>"  required="required">
  </td> 
  </tr>
  <tr>
  <td align="right"><b> address<b></td>
  <td>
   <input type="address" name="address" class="form-control" value="<?php if($action=="edit"){ echo $data['address'];} else { echo "";} ?>" required="required">
   </td>
  </tr>
  <tr>
  <td align="right"><b>email <b></td>
  <td>
   <input type="text" name="email" class="form-control" value="<?php if($action=="edit"){ echo $data['email'];} else { echo "";} ?>"  required="required">
   </td>
  </tr>
  <tr>
  <tr>
  ...
  <td>
   <input type="mobile " name="mobile" class="form-control" value="<?php if($action=="edit"){ echo $data['mobile'];} else { echo "";} ?>"  required="required">
   </td>
  </tr>
  <tr>
  <tr>
...
<!-- start of table header change-->
      ...
      <!-- End of table header change-->
			...
			<?php
       //start of typing  query
		$query  = "select * from basic_details order by id desc";
    //En of typing  query
$result = mysqli_query($conn,$query);
